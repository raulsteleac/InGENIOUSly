...
//receptionarea celorlalte mesaje
 ...
 if((buffer[1]&15)!=3 && (buffer[0]&15)==1)
          ...
          {
          reset=1;
           if ( (((buffer[3]>>4&15)==(conti->rfidwt>>4&15))&&((buffer[3]&15)==((conti->rfidwt&15)+3)%4) && (buffer[4]&15)!=3) || (((buffer[3]>>4&15)==(conti->rfidwt>>4&15)) && ((buffer[3]&15)==4) && ((conti->rfidwt&15)==3)  ) )
                    {
                     stopprio=1;
                    printf("\n\n\n STOP  %d  !!!!!!!!!!! \n\n\n",(((conti->rfidwt&15)+3)%4));
                    echipa=buffer[1];
       ...
       if(echipa==buffer[1] && ((buffer[3]>>4&15)==(conti->rfidwt>>4&15))&&((buffer[3]&15)==((conti->rfidwt&15)+1)%4))
     ...
     if((buffer[0]&15)==2)
     	...
     	if(buffer[1]==conti->semnatura[0] && buffer[2]==conti->semnatura[1]  && buffer[3]==conti->semnatura[2] && buffer[4]==conti->semnatura[3])
              ...
              reset=1;
              memset( conti, 0, sizeof(*conti) );
              memset( conti, 0, sizeof(*state) );
              memset( conti, 0, sizeof(*traseu) );
     					printf("M-AM OPRIT");
                    		conti->semnatura[0]=conti->semnatura[0]|'A';
                    		conti->semnatura[0]=conti->semnatura[0]<<4|'A';
                    		conti->semnatura[1]=conti->semnatura[1]|'B';
...
while(transmitterflag);
    letssend(&soct,&transmitterlen,&transmitter,conti,state,data,rfx,rfids,&flag1,&capat,&flagMiddle,wfrf);
printf("1\n");
    usleep(100000);
    }
}
