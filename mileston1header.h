...
void broadcastpermissioner(int *soc)
{
 ...
 int sec=0;
void letssend(int *soct,socklen_t * transmitterlen,struct sockaddr_in * transmitter,struct container *conti,char *state,uint8_t *data,uint32_t rfx,uint32_t rfids[25][18],int *flag1,int *flag2,int *flag3,char (*rfiddecoder)(uint32_t ,uint32_t [25][18] ),int ok)
{
 if(!ok)
 {
if(rfx!=0)
...
printf("  RFX : %X \n",rfx);
     if(conti->rfidwt!=state[3] &&( (conti->rfidwt>>4&15)!=0))
      {
